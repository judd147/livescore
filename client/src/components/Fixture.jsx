...
function Fixture() {
  ...
  async function fetchNewData() {
    ...
    // If no previous timestamp or if it's been more than 24 hours
    if (!lastFetchTimestamp || currentTime - parseInt(lastFetchTimestamp, 10) > 24 * 60 * 60 * 1000) {
        ...
        console.log('Data was fetched less than 24 hours ago. Skipping fetch.');
    }
  }
  ...
  async function updateData() {
    const currentTime = Date.now();
    const targetMatches = matchesFromDb.filter((match) => match.status !== "FT" && Date.parse(match.date) < currentTime);
  ...
  useEffect(() => {
    fetchNewData();
    // Update match data every 30 minutes
